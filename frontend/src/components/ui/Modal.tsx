      ...
      >
        ...
        {(title || description) && (
          <div className="flex items-start justify-between p-6 border-b border-warm-100">
            <div>
              {title && <h2 className="text-xl font-semibold text-stone-800">{title}</h2>}
              {description && <p className="mt-1 text-sm text-stone-500">{description}</p>}
            </div>
            <button
              onClick={onClose}
              className="ml-4 p-2 rounded-lg text-stone-400 hover:text-stone-600 hover:bg-warm-100 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
